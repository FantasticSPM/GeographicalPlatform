<template>
  <div class="base-map-control">
    ...
    <div
      class="base-map-list"
      :class="{
        'base-map-list-left': placement === 'left',
        'base-map-list-right': placement === 'right',
      }"
    >
      <div
...
const currentBaseMap = ref(
  currentId ? JSON.parse(currentId) : props.baseMapList[0],
);
...
function removeBaseMap() {
  ...
  if (layer) {
    ...
    window.map.getLayer(baseMapName + "_other") &&
      window.map.removeLayer(baseMapName + "_other");
...
function submit(data) {
  const cur = props.baseMapList.find((i) => i.id === "division");
  cur.data = data;
...
function getSetting(options) {
  if (options.id === "division") {
    ...
    if (code.endsWith("00000")) {
      ...
      defaultSettings.value.level =
        a.length === 2 ? levelMap.CITY : a.length === 3 ? levelMap.COUNTY : "";
...
.base-map-control {
  ...
  .current-base-map {
    ...
    box-shadow:
      0 3px 10px rgba(16, 47, 73, 0.2),
      0 1px 2px rgba(16, 47, 73, 0.12);
    cursor: pointer;
    transition:
    ...
    &:hover {
      border-color: #eaf7ff;
      box-shadow:
        0 6px 14px rgba(16, 47, 73, 0.26),
        0 2px 4px rgba(16, 47, 73, 0.14);
  ...
  .base-map-list {
    ...
    box-shadow:
      0 8px 20px rgba(24, 59, 91, 0.16),
      0 2px 5px rgba(24, 59, 91, 0.08);
    opacity: 0;
    visibility: hidden;
    pointer-events: none;
    white-space: nowrap;
